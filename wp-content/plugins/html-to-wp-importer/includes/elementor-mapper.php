...
class HTML_WP_Elementor_Mapper {
    ...
    public static function html_to_elementor_json($html) {
        ...
        if ($body) {
            $elementor_data = self::process_node_to_elementor($body);
        }
        
        return $elementor_data;
    }
    
    private static function process_node_to_elementor($node) {
        $sections = [];
        ...
        foreach ($node->childNodes as $child) {
            if ($child->nodeType === XML_ELEMENT_NODE) {
                ...
                if (self::is_section_element($tagName)) {
                    ...
                    $current_section = self::create_elementor_section($child);
                    $current_column = null;
                }
                // Handle column-level elements
                elseif (self::is_column_element($tagName)) {
                    if ($current_section) {
                        $current_column = self::create_elementor_column($child);
                        $current_section['elements'][] = $current_column;
                    }
                }
                // Handle widget-level elements
                else {
                    $widget = self::create_elementor_widget($child);
                    if ($widget) {
                        if ($current_column) {
                            ...
                            $current_section = self::create_default_elementor_section();
                            $current_section['elements'][] = $widget;
                            $sections[] = $current_section;
                            $current_section = null;
                        }
        ...
        return empty($sections) ? [self::create_default_elementor_section()] : $sections;
    }
    ...
    private static function create_elementor_section($node) {
        $section = self::make_element('section');
        
        // Add section settings
        $section['settings'] = self::get_section_settings($node);
    ...
    private static function create_elementor_column($node) {
        $column = self::make_element('column');
        
        // Add column settings
        $column['settings'] = self::get_column_settings($node);
        
        return $column;
    }
    
    private static function create_elementor_widget($node) {
        ...
        $widget = self::make_widget($widget_type);
        $widget['settings'] = self::get_widget_settings($node, $widget_type);
        
        return $widget;
    }
    
    private static function create_default_elementor_section() {
        $section = self::make_element('section');
        $section['settings'] = [
            'layout' => 'fullwidth',
    ...
    private static function make_element($type) {
        return [
            'id' => uniqid(),
            'elType' => $type,
            'settings' => [],
    ...
    private static function make_widget($type) {
        return [
            'id' => uniqid(),
            'elType' => 'widget',
            'widgetType' => $type,
